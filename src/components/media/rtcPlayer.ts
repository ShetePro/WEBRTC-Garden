import { RoomData, RtcIceResponse, RtcSdpResponse } from "@/types/room.ts";
...
export class RtcPlayer {
  peerList: Record<string, RTCPeerConnection | null>;
  roomData: RoomData;
  ...
  constructor() {
    ...
    this.roomData = computed<RoomData>(() => {
  ...
  createPeerConnection(user: User) {
    ...
    peer.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        this.socket.emit("__ice_candidate", {
  ...
  socketInit() {
    ...
    socket.on("offer", (v: RtcSdpResponse) => {
      ...
      if (this.peerList[v.userId])
        this.peerList[v.userId]?.setRemoteDescription(
          v.sdp,
          () => {
            this.peerList[v.userId]?.createAnswer().then((desc) => {
              this.peerList[v.userId]?.setLocalDescription(
                desc,
                () => {
                  console.log("给其他用户发送 answer", v.userId);
                  socket.emit("answer", {
                    sdp: this.peerList[v.userId]?.localDescription,
                    roomid: roomData.id,
                    userId: v.userId
                  });
                },
    ...
    socket.on("__ice_candidate", (v: RtcIceResponse) => {
      ...
      if (v.candidate && this.peerList[v.userId]) {
        this.peerList[v.userId]?.addIceCandidate(v.candidate);
      }
    });
